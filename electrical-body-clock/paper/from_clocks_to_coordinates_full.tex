...
statistical suppression). The shared axis \A\ was separately robust and
...
($r=\num{0.26}$). Supporting analyses posed related coordinate-based questions at other
biological scales---whole-body CT and brain MRI---using scale-appropriate representations rather
than the electrical (\A/\D)/signed-direction geometry itself; a separate exploratory evolutionary-genomics
...
validated regulatory findings. Biological age is not one clock but a direction in a state space.
\end{abstract}
...
and the structural (whole-body CT) and genomic (human-accelerated DNA) scales, with the skeleton as
...
were empirically nearly uncorrelated ($r=-\num{0.17}$), consistent with \A\ being orthogonal to
the contrast subspace by construction. External disagreement was lowest in atrial-fibrillation/flutter records,
...
prespecified, hash-locked, outcome-blind protocol (Methods). In the Aging Brain Cohort
(AABC / HCP-Aging Release~2; \num{1396} adults, \num{2878} visits, controlled access),
four separately trained, channel-specific brain-age clocks were fit on complementary imaging
modalities---cortical \emph{structure}, \emph{myelin} (T1w/T2w), \emph{perfusion} (arterial spin
labeling) and \emph{function} (resting-state fMRI)---the geometry was frozen, and only then was a
locomotor outcome opened. All four clocks recovered chronological age well on held-out data
(Pearson $r=\num{0.59}$, \num{0.51}, \num{0.51} and \num{0.50}), passing the
pre-specified $r\ge\num{0.20}$ gate. The shared axis is orthogonal to the contrast subspace by
construction, and the shared axis and disagreement radius were empirically nearly uncorrelated
($r=-\num{0.03}$; Figs.~\ref{fig:s5}--\ref{fig:s8}).

The prespecified primary test---whether baseline brain-channel disagreement predicts
...
pairings and not others, and which is an empirical question to be settled by prespecified
tests, not assumed.
...
older, 55--80~y) were processed end-to-end into four \emph{complementary} T1-derived
estimates of structural brain age---a deep brain-age regression, gray-matter fraction, the
ventricle--brain ratio and the brain-tissue fraction (Methods)---and a within-brain dispersion
coordinate was defined as the standard deviation across those four oriented, cohort-standardized
views: a simpler construct check of internal inconsistency, not the covariance-normalized
Mahalanobis radius \D\ used in the electrical, organ-system and AABC arms (Fig.~\ref{fig:lemon}). Because the age clock carries a domain-shift bias across the
...
result that poses related coordinate-based questions at other biological scales using
scale-appropriate representations, \emph{not} independent confirmation of the electrical
perturbation finding. This structural scale
...
\citep{pickhardt2020automated,kerber2023deeplearning}. This CT structural age
clock couples a single whole-body volume model with per-structure permutation
importance across the full TotalSegmentator label set, an approach the author is
not aware of having been reported previously.
...
\bigskip
\section{Part~III --- Supporting evolutionary-genomics screen: predicted skeletal-lineage effects in the Zoonomia HAR catalogue}
\label{sec:partiii}
...
phenotype. The supporting analyses do not instantiate one identical geometry or establish a causal
chain across scales. NHANES tests blood-biomarker system coordination, CT localises age-related
morphology, brain imaging tests disagreement across derived views, movement probes a
structure--function boundary, and the genomic screen prioritises predicted evolutionary regulatory
hypotheses --- reporting a null enrichment near skeletal disease loci. Their contribution is to
map where coordinate-based descriptions appear informative, null, or technically limited.
...
prespecified tests it showed no detectable association with structural disease, no
detectable change under two distinct acute physiological stressors, and---on the single
prespecified confirmatory test---no association with mortality beyond the shared axis
...
direction: applying the same shared-axis/disagreement construction---the identical
covariance-normalized Mahalanobis radius, separately calibrated on the brain cohort---to four
...
an entirely separate cohort and pipeline: across 220 LEMON structural scans, four
complementary T1-derived aging views agree in younger brains but diverge in older ones
...
older-appearing and $z$-scored across the cohort, and a within-brain dispersion coordinate was
defined as the standard deviation across a subject's four oriented $z$-scores---a mean-invariant
measure of internal inconsistency between the clocks, not distance from the cohort center. This is
a deliberately simpler construct check than the covariance-normalized Mahalanobis radius \D\ used
in the electrical, organ-system and AABC arms; it shares the shared-axis/disagreement motivation
but not the exact metric. The four views are complementary rather than statistically
independent: all are derived from the same T1-weighted acquisition and are therefore correlated,
so their disagreement measures within-modality inconsistency across analysis views, not agreement
between independently acquired measurements. Because the deep brain-age clock was trained on conventional
...
indices in \texttt{results/skeletome\_borzoi\_track\_indices.csv} --- are osteoblast (index~1319,
...
only as a forward-looking prioritisation aid. The blind \textit{GDF5}/GROW1 filter-logic control
--- variant identities and pass/reject calls, but no predicted effect direction --- was frozen prior
to scoring and is reported without post-hoc adjustment. The HAR call set, phyloP track, alignment
...
cohort were processed end-to-end into four \emph{complementary} T1-derived estimates of
...
same voxels that the four complementary T1-derived views read differently in older tissue; no
...
\begin{figure}[htbp]
  ...
  \caption[Exploratory evolutionary-genomics hypothesis screen of Human Accelerated Regions]{%
    ...
    \textbf{(c)} The central inferential result --- a null. The ten HARs within \num{25}~kb of an
...
and disagreement radius were empirically nearly uncorrelated (Fig.~\ref{fig:s7}), consistent with
the shared axis being orthogonal to the contrast subspace by construction, so the null
...
$\D_{\text{brain}}$ (standardized Mahalanobis contrast length) are empirically nearly uncorrelated
($r=-\num{0.03}$), consistent with $\A_{\text{brain}}$ being orthogonal to the contrast subspace by
construction. \textbf{(b)} Participants in the highest disagreement
