...
% Full 54-page technical manuscript; the 6-page judge cut is clocks_to_coordinates.tex.
...
\newcommand{\pendingbox}{%
  \begin{center}
  \setlength{\fboxrule}{1.2pt}%
  \fbox{\parbox{0.92\linewidth}{\centering
  \textbf{\textcolor{pendinggray}{CONFIRMATORY RESULT PENDING --- CODE-15 final-split reveal not yet executed.}}\\[3pt]
  \small The single pre-registered confirmatory test (primary M2-vs-M1 Cox
  likelihood-ratio test for \D\ on the locked CODE-15 final-test partition,
  $n\approx\num{75183}$) has \emph{not} been executed. No mortality value for any
  final-test patient has been read. This box will be replaced by the frozen result
  template only after the single authorized reveal. The development association
  (\S\ref{sec:mortality}) is reported as hypothesis-generating and model-freezing,
  explicitly \emph{not} as confirmatory evidence.}}
  \end{center}}
...
($r=\num{0.26}$). The same decomposition was applied as a supporting multiscale atlas across
whole-body CT, brain MRI and an \emph{in-silico} genomic screen. Biological age is not one clock
but a direction in a state space.
\end{abstract}
...
has not, to the authors' knowledge, been reported is (i) to train \emph{independent} age clocks for the individual
...
Third, in STAFF~III, acute ischemia did not change \D\ (inflation minus baseline
$+\num{0.14}$~SD, 95\% CI $[-\num{0.30},\num{0.57}]$, $p=\num{0.54}$; recovery
statistically equivalent to baseline by a two one-sided test at $\pm 1$~SD). The
...
direction was constructed, so the transportable ``atlas'' contains a single pre-registered
perturbation direction ($k=1$). The signed score was then standardized (mean $\mu$, SD $\sigma$) on the frozen healthy
...
penalized fit). In other words the frozen $I_{\text{Kr}}$ direction adds information about
...
marginally separable one. The effect confirmed independently in the larger Ningbo site
(\num{329} cases, OR \num{1.20}, $p=\num{0.002}$); Chapman--Shaoxing pointed the same way but
...
and only the QRS-clock read its interval appreciably ($\rho=+\num{0.20}$). The AV clock---the
strongest mortality channel---is if anything \emph{weakly negatively} related to the PR
interval, so its prognostic signal is not a re-badging of classical PR prolongation
...
($\mathrm{ICC}=\num{0.31}$), which is consistent with a genuine signal rather than a
...
\subsection{The same decomposition works across the body}
...
\numrange{0.0029}{0.0054}). The concordance ladder rose from \num{0.8147} (age and
...
the whole body is, to the authors' knowledge, a new observation about the structure of
biological age.
...
\citep{pickhardt2020automated,kerber2023deeplearning}. To the authors'
...
\num{81} neural comparator tracks. The reported score is the skeletal minus neural accessibility
delta, so that pleiotropic changes registering in both lineages cancel and only effects specific to
the skeletal readouts are retained. Across the substitution catalogue the score spans
...
To the authors' knowledge this is the first \textit{in-silico} variant-effect screen to score the
...
in full because the effect is conditional (suppression). Prespecified robustness comprised a Firth
...
information against QTc, \A\ and \D. A reliable-QT subgroup (QTc \SIrange{350}{600}{\milli\second},
...
inferred beyond the track's own annotation. The skeletal-specific effect is the mean skeletal delta
minus the mean neural delta, so shared pleiotropic effects cancel and only skeletal-specific
accessibility changes are scored. Tracks were combined on the model's native predicted-coverage
